-- ============================================================================
-- Luman Weather - client interface (NUI)
--
...
AddEventHandler("luman-weather:toggleForecast", function()
    ...
    CreateThread(function()
        while forecastIsDisplayed do
            TriggerServerEvent("luman-weather:requestUpdatedForecast")
            Wait(1000)
        end
    end)
...
AddEventHandler("luman-weather:openAdminUi", function(weather, time, timescale, windDirection, windSpeed, syncDelay)
    ...
    CreateThread(function()
        while adminUiIsOpen do
            TriggerServerEvent("luman-weather:requestUpdatedAdminUi")
            Wait(1000)
        end
    end)
end)
...
RegisterNUICallback("setTime", function(data, cb)
    TriggerServerEvent("luman-weather:setTime", data.day, data.hour, data.min, data.sec, data.transition, data.freeze)
    cb({})
end)

RegisterNUICallback("setTimescale", function(data, cb)
    TriggerServerEvent("luman-weather:setTimescale", data.timescale * 1.0)
    cb({})
end)

RegisterNUICallback("setWeather", function(data, cb)
    TriggerServerEvent("luman-weather:setWeather", data.weather, data.transition * 1.0, data.freeze, data.permanentSnow)
    cb({})
end)

RegisterNUICallback("setWind", function(data, cb)
    TriggerServerEvent("luman-weather:setWind", data.windDirection * 1.0, data.windSpeed * 1.0, data.freeze)
    cb({})
end)

RegisterNUICallback("setSyncDelay", function(data, cb)
    TriggerServerEvent("luman-weather:setSyncDelay", data.syncDelay)
    cb({})
end)
